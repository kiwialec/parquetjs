...
function encodeDataPage(column, values, rlevels, dlevels, statistics) {
  ...
  if (column.rLevelMax > 0) {
    rLevelsBuf = encodeValues(
      ...
      rlevels, { bitWidth: parquet_util.getBitWidth(column.rLevelMax) });
  }
  ...
  if (column.dLevelMax > 0) {
    dLevelsBuf = encodeValues(
      PARQUET_RDLVL_TYPE,
      PARQUET_RDLVL_ENCODING,
      dlevels, { bitWidth: parquet_util.getBitWidth(column.dLevelMax) });
  }
 ...
 */
 function encodeFooter(schema, rowCount, rowGroups) {
  ...
  metadata.created_by = 'BoilingData';
  ...
  console.log("VIRTULPARQ:", { metadataEncodedLen: metadataEncoded.length })
