...
class ParquetCursor {
  ...
  async getDecompressedColumnChunk(colChunk){
    return await this.envelopeReader.decompressColumnChunk(this.schema, colChunk);
  }
  /**
...
class ParquetReader {
   ...
   */
   ...
   static async useS3Object(s3, fn, params, options) {
    let envelopeReader = await ParquetEnvelopeReader.useS3Object(s3, fn, params, options);
  ...
  static async openEnvelopeReader(envelopeReader, opts) {
    ...
    try {
      ...
      return new ParquetReader(metadata.metadata, envelopeReader, opts);
    } catch (err) {
...
class ParquetEnvelopeReader {
  ...
  static async useS3Object(s3, fn, params, options) {
    ...
    const fileStat = async () => fn.headS3Object(s3, { s3Params: { Bucket, Key } }).then(d => d.ContentLength);

    const readFn = async (startByte, length, _file) => {

      const endByte = startByte + length - 1;
      return fn.getRangeFromS3Object(s3, { startByte, endByte, s3Params: { Bucket, Key } });
    };
    const closeFn = () => ({});
  ...
  async readPage(column, page, records, opts) {
    ...
    let data = {
      columnData: {
        [chunk.column.meta_data.path_in_schema.join(',')]: chunk
      }
    };
  ...
  async decompressColumnChunk(schema, colChunk) {
    if (colChunk.file_path !== null) {
      throw 'external references are not supported';
    }

    let field = colChunk.meta_data.path_in_schema[0];
    let type = parquet_util.getThriftEnum(
        parquet_thrift.Type,
        colChunk.meta_data.type);
  ...
  async readFooter() {
    ...
    return {
      trailerSize: trailerLen,
      fileSize: this.fileSize,
      metadataSize: metadataBuf.length,
      metadataOffset,
      metadata,
    };
  }
...
function decodeDataPage(cursor, header, opts) {
  ...
  if (opts.rLevelMax > 0) {
    rLevels = decodeValues(
      ...
      rLevelEncoding,
      valuesBufCursor,
      valueCount, { bitWidth: parquet_util.getBitWidth(opts.rLevelMax) });
  } else {
    rLevels.fill(0);
  }
  ...
  if (opts.dLevelMax > 0) {
    dLevels = decodeValues(
      ...
      dLevelEncoding,
      valuesBufCursor,
      valueCount, { bitWidth: parquet_util.getBitWidth(opts.dLevelMax) });
  } else {
    dLevels.fill(0);
  }
