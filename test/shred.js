...
describe('ParquetShredder', function () {
  ...
  it('should materialize a nested record with scalar repeated fields', function () {
    ...
    assert.deepEqual(
      records[0], { name: "apple", price: [23.5] });

    assert.deepEqual(
      records[1], { name: "orange", price: [17, 23] });

    assert.deepEqual(
      records[2], { name: "kiwi", price: [99, 100] });

    assert.deepEqual(
      records[3], { name: "banana", price: [42] });
  });
  ...
  it('should materialize a nested record with nested repeated fields', function () {
    ...
    assert.deepEqual(
      records[0], { name: "apple", stock: [{ quantity: [10], warehouse: "A" }, { quantity: [20], warehouse: "B" }], price: 23.5 });

    assert.deepEqual(
      records[1], { name: "orange", stock: [{ quantity: [50, 75], warehouse: "X" }], price: 17.0 });

    assert.deepEqual(
      records[2], { name: "kiwi", price: 99.0 });

    assert.deepEqual(
      records[3], { name: "banana", stock: [{ warehouse: "C" }], price: 42.0 });
  });
